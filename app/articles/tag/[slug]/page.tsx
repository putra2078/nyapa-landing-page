...
import apiService, { Article, TagSnapshot } from "../../../service/api";
...
export default function ArticlesByTagPage() {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      if (items.length > 0) {
        const tagObj = items[0].tags?.find((t: TagSnapshot) => t.slug === slug);
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex-grow pt-24 pb-12">
        ...
        <div className="mb-12">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mt-2">
            Artikel dengan tag:{" "}
            <span className="text-[#FF5100]">#{tagName || slug}</span>
          </h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100">
            ...
            <Link
              href="/articles"
              className="mt-4 inline-block text-[#FF5100] font-bold hover:underline"
            >
              Lihat semua artikel
            </Link>
          </div>
