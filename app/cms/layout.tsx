        ...
        <div className="flex h-screen bg-[#f4f5f7] font-sans overflow-hidden text-sm">
            ...
            <div className="w-[80px] bg-[#1a1c23] flex flex-col items-center py-4 text-gray-400 border-r border-[#2a2c35] flex-shrink-0">
                ...
                <nav className="flex-1 w-full space-y-2">
                    <Link href="/cms">
                        <NavItem icon={<rect x="3" y="3" width="18" height="18" rx="2" strokeWidth="2" />} label="Dashboard" active={isDashboard} />
                    </Link>
                    <Link href="/cms/artikel">
                        <NavItem icon={<><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" strokeWidth="2" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" strokeWidth="2" /></>} label="Content" active={isArtikel} />
                    </Link>
                </nav>

                {/* Bottom Nav */}
                <div className="w-full">
                    <NavItem icon={<path d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" strokeWidth="2" />} label="Logout" />
                </div>
            </div>
                ...
                <div className="flex-1 overflow-y-auto pb-4">
                    <ul className="space-y-1 px-2">
                        ...
                        <Link href="/cms/artikel/create">
                            <SidebarLink label="Create Article" active={pathname === "/cms/artikel/create"} />
                        </Link>
                    </ul>
                </div>
            </div>
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
    return (
        <div className={`w-full flex-col items-center justify-center py-3 cursor-pointer group flex relative ${active ? 'bg-[#2a2c35] text-white' : 'hover:text-white'}`}>
            ...
            {active && <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-blue-500 rounded-r"></div>}
        </div>
    );
}

function SidebarLink({ label, active = false, icon }: { label: string, active?: boolean, icon?: React.ReactNode }) {
    return (
        <li className={`flex items-center px-4 py-2 rounded-md cursor-pointer text-sm ${active ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-600 hover:bg-gray-50'}`}>
            {icon}
            {label}
        </li>
    );
